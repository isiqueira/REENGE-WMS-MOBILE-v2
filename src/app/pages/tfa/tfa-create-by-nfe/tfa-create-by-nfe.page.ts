...
@Component({
  selector: 'app-tfa-create-by-nfe',
  templateUrl: './tfa-create-by-nfe.page.html',
  styleUrls: ['./tfa-create-by-nfe.page.scss'],
  standalone: false
})
export class TfaCreateByNfePage implements OnInit {
  ...
  async ok() {
    ...
    if (this.tfaCreate.Id && this.tfaCreate.Id !== 0) {
      this.tfaService.editTfaByNfe(this.tfaCreate).subscribe({
        next: () => {
      ...
      this.basePage.dismissLoading().then(() => {
        this.getFailureTypesAndSave();
      });
    }
  }

  async getFailureTypesAndSave() {
    await this.basePage.newLoading();
    this.tfaService.getTfaFailureTypes().subscribe({
      next: async (failureTypes) => {
        await this.basePage.dismissLoading();
        const selected = await this.basePage.newGetBarcode(TypeBarcode.TfaFailure, { AlternativesFailureTypes: failureTypes });
        if (!selected) return;
        this.tfaCreate.TfaFailureId = selected;
        await this.saveTfa();
      },
      error: async (err) => {
        await this.basePage.dismissLoading();
        this.basePage.newAlert('Erro', err?.error?.Detail?.message || 'Erro ao obter tipos de avaria.');
      }
    });
  }
  ...
  async saveTfa() {
    await this.basePage.newLoading();
    this.tfaService.saveTfaByNfe(this.tfaCreate).subscribe({
      next: (data) => {
        this.basePage.dismissLoading().then(() => {
